...
class RentalSpiderSpider(scrapy.Spider):
    ...
    def parse_apartment_details(self, response):
        ...
        price_per_msq=response.xpath("//span[text()='Alquiler//M² de construcción:']/following-sibling::span/text()").extract_first()
        bedroom=response.xpath("//span[text()='Recamaras:']/following-sibling::span/text()").extract_first()
        if msq is None:
            msq=response.xpath("//span[text()='Tamaño del Lote:']/following-sibling::span/text()").extract_first()
